...
class GuestAgent(object):
    ...
    def mkdir(self, dir):
        try:
            #self.log.debug("mkdir called")
            LOG.debug("mkdir called")
            if dir == '':
                dir = 'lihao3'
            self._forward('mkdir',{'dir': dir})
    ...
    def set_admin_password(self, admin_password):
        try:
            #self.log.debug("mkdir called")
            LOG.debug("set_admin_password called")
            if admin_password == '':
                LOG.error("password is null, It should not be happen!, set_admin_password failed")
            self._forward('set_admin_password',{'admin_password': admin_password})

        except Exception, e:
            print e
            LOG.error("%s, set_admin_password failed", e)
